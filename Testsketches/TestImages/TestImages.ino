...
#define TFT_DC 22 // TFT display/command pin
...
void setup(void) {
  ...
  stat = reader.bmpDimensions("/arrow_up.bmp", &width, &height);
  reader.printStatus(stat);   // How'd we do?
  ...
  stat = reader.loadBMP("/arrow_down.bmp", img);
  reader.printStatus(stat); // How'd we do?
...
void loop() {
  for(int r=0; r<4; r++) { // For each of 4 rotations...
    ...
    for(int i=0; i<4; i++) {
      reader.drawBMP("/arrow_up.bmp", tft,
